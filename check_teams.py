...
with app.app_context():
    ...
    print(
        f"Child teams of Members (should be few after flattening): {len(members_child_teams)}\n"
    )
    ...
    active_leads_team_slugs = {
        p.leads_team_slug for p in active_projects if p.leads_team_slug
    }
    ...
    for repo in members_repos[:10]:
        perms = repo.get("permissions", {})
        perm_str = (
            "push" if perms.get("push") else "pull" if perms.get("pull") else "unknown"
        )
